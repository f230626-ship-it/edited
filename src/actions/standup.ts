...
export async function getStandupEntries(
  ...
  if (!entries || entries.length === 0) {
    return {
      entries: [],
      label,
      stats: { totalEntries: 0, totalTasks: 0, totalBlockers: 0, avgScore: 0 },
    };
  }

  const employeeIds = [
    ...new Set(entries.filter((e) => e.employee_id).map((e) => e.employee_id!)),
  ];
  const employeeMap = new Map<
    string,
    { name: string; photo: string | null; designation: string | null }
  >();
  ...
  const mappedEntries: StandupEntry[] = entries.map((e) => ({
    ...
    employee_name: e.employee_id
      ? employeeMap.get(e.employee_id)?.name || "Unknown"
      : "Unknown",
    employee_photo: e.employee_id
      ? employeeMap.get(e.employee_id)?.photo || null
      : null,
    employee_designation: e.employee_id
      ? employeeMap.get(e.employee_id)?.designation || null
      : null,
    raw_text: e.raw_text,
  ...
  const totalTasks = mappedEntries.reduce(
    (s, e) => s + e.completed.length + e.in_progress.length,
    0
  );
  ...
  const avgScore =
    mappedEntries.length > 0
      ? Math.round(
          mappedEntries.reduce((s, e) => s + e.performance_score, 0) /
